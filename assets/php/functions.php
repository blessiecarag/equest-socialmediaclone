<?php
require_once 'config.php'; //include config.php to connect the database
$db = mysqli_connect(DB_HOST,DB_USER,DB_PASS,DB_NAME) or die("Error! Database failed to connect");

//for displaying different pages based on the index code
...
//for retrieving user id for messages
...
//for getting messages from users
...
// for checking if messages is read by user
...
// for retrieving time of message sent
...
//for unblocking different users
...
//for displaying like status (user who liked the post)
...
//for liking different posts with addition to notifying the user who posted it
...
//for posting comments on other users' posts
...
// for counting the number of unread notifications
...
//for displaying the count of likers per each post
...
//for unliking other users' posts
...
// for unfollowing other users and notifying them
...
//for displaying errors using bootstrap alerts
...
//for displaying previous form data if the back button is used by the user
...
//for confirming if the email is already registered
...
//for confirming if the username is already registered
...
//for confirming if username is taken
...
//for checking the user details based on data entered from login form
...
//for retrieving user data via id
...
//for handling follow suggested for each user
...
//for confirming follow status if the user is followed by a certain user or not
...
//for confirming block status if the user is blocked by a certain user or not
...
// for checking the block list of the user
...
//for displaying random follow suggestions of existing users
...
//for counting the number of followers of each user
...
//for counting the number of following of each user
...
//for handling the user searches
...
//for retrieving user data via username
...
//for retrieving all details of post data
function getPost(){
    global $db;
...
//for displaying different posts from other users
...
//for handling the creation of new users
...
//for handling forms if the user failed to fill up certain fields then they will be notified by these messages
function validateUpdateForm($form_data,$image_data){
    ...
    $response['status']=true;
        ...
        if(!$form_data['username']){
            $response['msg']="No username entered";
            $response['status']=false;
            $response['field']='username';
        }
        
        if(!$form_data['last_name']){
            $response['msg']="No last entered";
            $response['status']=false;
            $response['field']='last_name';
        }
        if(!$form_data['first_name']){
            $response['msg']="No first name entered";
            $response['status']=false;
            $response['field']='first_name';
        }
       ...
       if($image_data['name']){
        ...
        if($size>1000){
            $response['msg']="Only images less then 1 mb are allowed";
            $response['status']=false;
            $response['field']='profile_pic';
        }
       }
...
return mysqli_query($db,$query);
    ...
    //for post validation if whether or not an image is included or if it's the right file size
function validatePostImage($image_data){
    ...
    //for creating new posts by the user
